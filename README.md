...
#### 字典函数创建
在python中，`dict()`为字典的函数。既然为函数括号就应该为小括号`()`，并非是字典的大括号`{}`。创建时也需要输入“键”、`:`和“值”，并且需要一一对应。**注意用该方法时，不能往“键”上面加引号**（后面的函数部分会讲解为什么不能加引号）。例如：
```python
...
#### `update([other])`方法
方法可以支持同时传入多个键值对，也可以直接传入另外一个字典，或者一个包含键值对的一个可迭代对象。当传入的参数中包含字典中已经存在的键，那么传入的键值对会覆盖原有的键值对来达到修改的目的。由于支持的参数种类很多，因此修改也有多种写法。例如：
```python
a = dict.fromkeys("xyl",250)
...
fun1()
```
很显然，运行的结果不是`fun2()`直接运行的结果，而是一个对`fun2`的引用。另外调用函数就是函数名加上小括号，那么就可以添加两个括号也能表示对`fun2`的引用。例如：
```python
def fun1():
    x = 520
    def fun2():
...
open("Python\《零基础入门学习Python》\AAA.txt","w") # 写入模式
```
